...
public class EditPageController {
	...
	public void fillinRecipeView(Recipe r) {
		...
		try {
			...
			showAlert(Alert.AlertType.ERROR, "Error", "Load failed", "Unable to load recipe image!");
			return;
		}
	}
	...
	public void setEditView() {
		...
		quantityCol.setOnEditCommit((TableColumn.CellEditEvent<Ingredient, Integer> t) -> {
			((Ingredient) t.getTableView().getItems().get(t.getTablePosition().getRow())).setQuantity(t.getNewValue());
		});
		ingDescriptionCol.setCellFactory(TextFieldTableCell.forTableColumn());
		ingDescriptionCol.setOnEditCommit((TableColumn.CellEditEvent<Ingredient, String> t) -> {
			((Ingredient) t.getTableView().getItems().get(t.getTablePosition().getRow()))
					.setDescription(t.getNewValue());
		});

		instructionTable.setEditable(true);
		insDescriptionCol.setCellFactory(TextFieldTableCell.forTableColumn());
		insDescriptionCol.setOnEditCommit((TableColumn.CellEditEvent<Instruction, String> t) -> {
			((Instruction) t.getTableView().getItems().get(t.getTablePosition().getRow()))
					.setDescription(t.getNewValue());
		});
	}
